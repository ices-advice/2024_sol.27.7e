...
plot_stock_comparison <- function(
  ...
  res_df$qname <- factor(res_df$qname, unique(res_df$qname))
  ...
  if (isTRUE(any(!is.na(refpts)))) {
    ...
    df_refpts$qname <- factor(df_refpts$qname, levels = unique(res_df$qname))
...
plot_stock <- function(stk, rec_shade_last = FALSE, 
                       Fpa, Flim, Fmsy, Bpa, Blim, MSYBtrigger,
                       catch_factor = 1, catch_units = units(catch(stk)),
                       rec_factor = 1000, rec_units = units(rec(stk)),
                       ssb_factor = 1000, ssb_units = units(ssb(stk)),
                       yr_min, yr_max, forecast = FALSE) {
  
  ### year range
  if (missing(yr_min))
    yr_min <- range(stk)[["minyear"]] - 1
  if (missing(yr_max))
    yr_max <- range(stk)[["maxyear"]] + 1
  
  ### catch
  df_catch <- as.data.frame(FLQuants(landings = landings(stk),
                                     discards = discards(stk))) %>%
    mutate(data = data/catch_factor) %>%
    mutate(qname = factor(qname, levels = c("discards", "landings")))
  if (isTRUE(forecast)) df_catch <- df_catch %>% filter(year < max(year))
  y_label_catch <- paste0("Catches in ", catch_units)
  max_catch <- df_catch %>%
    group_by(year) %>%
    summarise(catch = sum(data)) %>%
    select(catch) %>%
    max()
  p_catch <- ggplot() +
    geom_col(data = df_catch,
                      ...
                      na.rm = TRUE) +
    scale_fill_manual("", values = c(landings = "#002b5f", discards = "#28b3e8")) + 
    coord_cartesian(ylim = c(0, max_catch * 1.1), 
                             xlim = c(yr_min - 1, yr_max + 1), 
                             expand = FALSE) +
    labs(x = "", y = y_label_catch, 
         title = "Catches") +
  ...
  ### recruitment
  df_rec <- as.data.frame(FLQuants(Recruitment = rec(stk))) %>%
    mutate(data = data/rec_factor)
  y_label_rec <- paste0("Recruitment in ", rec_units)
  if (isTRUE(forecast) & isTRUE(rec_shade_last)) {
    df_rec_fc <- df_rec %>% filter(year == max(year))
    df_rec <- df_rec %>% filter(year < max(year))
  }
  max_rec <- df_rec %>%
    group_by(year) %>%
    summarise(rec = sum(data)) %>%
    select(rec) %>%
    max()
  p_rec <- ggplot() +
    ...
    coord_cartesian(ylim = c(0, max_rec * 1.1), 
                    xlim = c(yr_min - 1, yr_max + 1), 
                    expand = FALSE) +
    labs(x = "", y = y_label_rec, 
         title = paste0("Recruitment (age ", range(stk)[["min"]], ")")) +
    theme_bw(base_size = 8) +
    theme(axis.title.y = element_text(face = "bold"),
          axis.title.x = element_blank(),
          legend.position = "bottom",
  ...
  if (isTRUE(forecast) & isTRUE(rec_shade_last)) {
    p_rec <- p_rec +
      geom_col(data = df_rec_fc,
               aes(x = year, y = data),
               na.rm = TRUE, fill = "#92defb")
  }
  
  ### fbar
  df_fbar <- as.data.frame(FLQuants(fbar = fbar(stk)))
  if (isTRUE(forecast)) df_fbar <- df_fbar %>% filter(year < max(year))
  df_fbar_refs <- data.frame(ref = c("F[pa]", "F[lim]", "F[MSY]"),
                             value = c(Fpa, Flim, Fmsy)) %>%
    mutate(ref = factor(ref, levels = c("F[pa]", "F[lim]", "F[MSY]")))
  y_label_fbar <- paste0("F (ages ", range(stk)[["minfbar"]], "-",
                         range(stk)[["maxfbar"]], ")")
  max_fbar <- df_fbar %>%
    select(data) %>%
    max()
  p_fbar <- ggplot() +
    geom_line(data = df_fbar,
             aes(x = year, y = data),
             na.rm = TRUE, colour = "#ed5f26") +
    geom_hline(data = df_fbar_refs, 
               aes(yintercept = value, colour = ref, linetype = ref)) +
    scale_colour_manual("", labels = scales::parse_format(),
                        values = c("F[pa]" = "#000000", "F[lim]" = "#000000", 
                                   "F[MSY]" = "#4297ff")) +
    scale_linetype_manual("", labels = scales::parse_format(),
                          values = c("F[pa]" = 1111, "F[lim]" = "dashed", 
                                     "F[MSY]" = "solid")) +
    coord_cartesian(ylim = c(0, max_fbar * 1.1), 
                    xlim = c(yr_min - 1, yr_max + 1), 
                    expand = FALSE) +
    labs(x = "", y = y_label_fbar, 
         title = paste0("F")) +
    theme_bw(base_size = 8) +
    theme(axis.title.y = element_text(face = "bold"),
          axis.title.x = element_blank(),
  ...
  ### SSB
  df_ssb <- as.data.frame(FLQuants(ssb = ssb(stk))) %>%
    mutate(data = data/ssb_factor)
  df_ssb_refs <- data.frame(ref = c("B[pa]", "B[lim]", "MSYB[trigger]"),
                             value = c(Bpa, Blim, MSYBtrigger)) %>%
    mutate(ref = factor(ref, levels = c("B[pa]", "B[lim]", "MSYB[trigger]")))
  y_label_ssb <- paste0("SSB in ", ssb_units)
  max_ssb <- df_ssb %>%
    select(data) %>%
    max()
  p_ssb <- ggplot() +
    geom_line(data = df_ssb,
              aes(x = year, y = data),
              na.rm = TRUE, colour = "#047c6c") +
    geom_hline(data = df_ssb_refs, 
               aes(yintercept = value, colour = ref, linetype = ref)) +
    scale_colour_manual("", labels = scales::parse_format(),
                        values = c("B[pa]" = "#000000", "B[lim]" = "#000000", 
                                   "MSYB[trigger]" = "#4297ff")) +
    scale_linetype_manual("", labels = scales::parse_format(),
                          values = c("B[pa]" = 1111, "B[lim]" = "dashed", 
                                     "MSYB[trigger]" = "solid")) +
    coord_cartesian(ylim = c(0, max_ssb * 1.1), 
                    xlim = c(yr_min - 1, yr_max + 1), 
                    expand = FALSE) +
    labs(x = "", y = y_label_ssb, 
         title = paste0("SSB")) +
    theme_bw(base_size = 8) +
    theme(axis.title.y = element_text(face = "bold"),
          axis.title.x = element_blank(),
